...
" ---------------------------------------------------------------------
"  Yacc Clusters: {{{1
syn cluster yaccInitCluster	contains=yaccKey,yaccKeyActn,yaccBrkt,yaccType,yaccString,yaccUnionStart,yaccHeader2,yaccComment,yaccDefines,yaccParseParam,yaccParseOption,yaccPrinter,yaccDestructor,yaccCodeBlock,yaccCodeSpec
...
" ---------------------------------------------------------------------
" Yacc Commands: {{{1
syn	match	yaccDefines	'^%define' skipwhite nextgroup=yaccDefinesParam
syn	match	yaccDefinesParam '[a-z.]\+' skipwhite nextgroup=yaccDefinesParamStr
syn	region	yaccDefinesParamStr	contained matchgroup=Delimiter	start='{'	end='}'	contains=cStructure
syn	match	yaccParseParam	'%\(parse\|lex\)-param\>'		skipwhite	nextgroup=yaccParseParamStr
syn	match	yaccParseOption '%\%(api\.pure\|pure-parser\|locations\|error-verbose\|debug\|glr-parser\|expect-rr\)\>'
...
syn	match	yaccKey	"^\s*%\(token\|type\|left\|right\|start\|ident\|nonassoc\|precedence\)\>"	contained
...
syn	match	yaccPrinter	"^%printer"	skipwhite skipnl nextgroup=yaccUnion	contained
syn	match	yaccDestructor	"^%destructor"	skipwhite skipnl nextgroup=yaccUnion	contained
syn	match	yaccCodeBlock	"^%code"	skipwhite skipnl nextgroup=yaccCodeSpec	contained
SynFold syn	match	yaccCodeSpec	"\s\(provides\|requires\|top\|\)" skipwhite skipnl nextgroup=yaccUnion	contained
...
if !exists("did_yacc_syn_inits")
  ...
  hi def link yaccDefines	yaccDefinesParam
  hi def link yaccDefinesParam cDefine
  ...
  hi def link yaccPrinter	yaccKey
  hi def link yaccDestructor	yaccKey
  hi def link yaccCodeBlock	yaccKey
  hi def link yaccCodeSpec	String
